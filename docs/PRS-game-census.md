...
The implemented P3 read service shares `queries.py` across HTML and versioned APIs. Catalog search opts into `scope=catalog` on `/api/v1/apps`; the enrolled-list default remains compatible. Known untracked details carry null player/tracking fields and separate catalog provenance, while unknown IDs return 404. `/api/v1/rankings` restricts ranks to fresh configured-cohort observations and reports each exclusion class. `/api/v1/compare` retains every selected known app in a common half-open UTC window, with per-series availability and coverage; unsupported or untracked sources are explicit rather than omitted. `web.max_compare_apps`, `web.max_compare_points` and `web.max_page_size` are implemented settings owned by `config.py`. HTML uses the same time and player-count scales and unique keyboard-accessible chart/table labels. Page loads perform no collection, and local refresh polling is single-flight with a configured refresh-interval deadline and visibility/lifecycle cancellation. P3 acceptance evidence and outstanding cohort/live-source gates belong to the checklist and walkthrough. [FR-03, FR-05, FR-06, NFR-01, NFR-03]

The implemented opt-in cohort policy is owned by `config.py` and `cohort.py`. `tracking.app_ids` are pinned while `cohort.enabled` is true. Bounded, repeated fresh player observations and distinct high/low thresholds control promotion/demotion; minimum residence, reconciliation intervals and replacement limits bound churn. A wrapping catalog cursor reserves exploration slots without inferring a missing count as zero. Shared scheduler admission includes discovery reserves and extra watched replacement-run attempts. `cohort plan/status/reconcile --once --dry-run|--apply` are direct inspectable interfaces; apply holds the collection lock and requires disabled scheduling. [FR-04, FR-05, NFR-01, NFR-03]

Additive `008_cohort.sql` retains canonical adoption decisions in `cohort_event` and tracking ends in `tracking_stop`. Recorded policy, roles, candidate IDs and player-capture references make each decision inspectable without copying observed counts into configuration. Configuration remains unchanged; runtime consumers resolve the last matching adoption. Disabled-policy reconciliation closes exploration intervals before static collection resumes. Removed members retain observations and historical intervals; reopening creates another interval. Start/end invalidation makes persisted rollup coverage rebuild from those retained intervals. `latest_app_name` is a derived view shared by enrolled summaries and search; a generated enrollment label cannot override an acquired Steam name. Public status exposes typed cohort roles/adoption state and stopped-history labels. Full backup manifests and scratch restore include the new ledgers. Live catalog and next-cohort acceptance remain separate checklist gates. [FR-02–FR-06, FR-10, NFR-01–NFR-03]
