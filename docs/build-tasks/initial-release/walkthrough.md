...
The first remote CI run exposed the Linux bind-mount ownership gap: config init created a mode-0600 file as container root, while the host runner then needed to read it. The [remote failure](evidence/independent-ci-first.txt), [failing regression](evidence/linux-config-owner-before.txt), and [same passing regression](evidence/linux-config-owner-after.txt) are retained. The wrapper now supplies the invoking UID/GID for POSIX configuration generation; permissions stay private. CI also retains setup output if startup fails. A rerun is required before full integration acceptance.


Remote CI on `c6b7dd5` completed clean Linux setup, **514 tests passed** (two dependency deprecation warnings), two isolated wheel builds and installed-package smoke checks. [Full unedited CI log](evidence/independent-ci-success.txt), [GitHub run](https://github.com/briandgibby/game-census/actions/runs/34176400077).

The independent Windows/Linux artifact comparison then [failed](evidence/release-cross-platform-before.txt). ZIP member inspection isolated working-copy CRLF line endings; after normalizing those, [comparison still failed](evidence/release-cross-platform-after.txt) because Windows/Linux ZIP platform metadata and zlib implementations differed. The build generator now normalizes only disposable source copies to the repository's LF policy and writes canonical uncompressed wheel ZIPs with fixed Unix metadata. It verifies every member's contents before retaining the canonical output. [The same comparison command passed](evidence/release-cross-platform-final.txt): two installed Windows builds matched the canonicalized independently built Linux artifact, SHA-256 `f5be91d581294e8e69b4260f0c1057aeb2b35ebab3a0ec01e1cb1e4c7e899301`. Original Hatch outputs remain in each build's `hatch` directory.

`tools/dev.py test-artifact --wheel PATH` installs that actual wheel in an isolated container with `/app/src` excluded, asserts its installed import location, and runs retained-scratch historical upgrade, recovery and enrichment tests. The generated CI workflow runs this extra artifact verification and retains its unedited output. It contacts no Steam endpoint and does not touch a collection instance.


### Independent implementation closeout

[Final CI on application/build commit `8ca0f23`](https://github.com/briandgibby/game-census/actions/runs/34177245035) completed every step successfully. [The unedited source/database output](evidence/independent-ci-suite.txt) ends with `514 passed, 2 warnings in 121.87s (0:02:01)`. [The installed-wheel output](evidence/independent-ci-artifact-tests.txt) ends with `63 passed, 2 warnings in 17.33s`. That second command imported the wheel from site-packages with `/app/src` excluded and exercised historical migrations, retained captures, native scratch restores and enrichment replay. The two warnings are the pinned Starlette/httpx and AnyIO deprecations; the CI actions also report their Node 20 targets being run on Node 24. No dependency upgrade was performed implicitly.

Both final Linux canonical wheels have SHA-256 `f5be91d581294e8e69b4260f0c1057aeb2b35ebab3a0ec01e1cb1e4c7e899301`, identical to the Windows canonical outputs. [Full final CI log](evidence/independent-ci-final.txt) retains clean setup, source tests, pinned artifact builds, installation and artifact-upgrade commands. Host packet validation reported zero errors and warnings, and the generated plan checks passed. The source feature branch is pushed; no PR was opened. The main and original canary worktrees remained clean.

All canary-independent implementation items are complete. No P3/P4 live-canary success, full-size reference-machine performance, off-site recovery policy, disk reclamation or public deployment is claimed. Docker recovery and disabling the interrupted canary's retained admission remain operator/runtime follow-up, not a reason to rerun or widen collection automatically.

### P3 canary restart — 2026-09-08

The user explicitly authorized starting a fresh canary after Docker recovered. The prior epoch-1 admission was disabled at `2026-09-08T02:40:19.746413Z`. [Read-only runtime and ledger inspection](evidence/p3-restart/preflight-v2.txt), produced by the retained [diagnostic command](evidence/p3-restart/preflight-v2.txt.json) and [script](evidence/p3-restart/preflight.py), found healthy web/database containers, no reported OOM kill or container restart, schema 9, zero collection locks and the unchanged tested image. The interrupted run retained 39 successful player attempts and 39 successful Store attempts, with no failed or uncertain results. Its completion report is absent. Its actual enable-to-disable interval has 28,153.026315 tracked app-seconds and 44.7802% freshness; it is not a completed or accepted 24-hour canary. The underlying Docker/database interruption remains unexplained; these checks establish current readiness, not a root-cause fix.

The [admitted plan](evidence/p3-restart/plan.txt) and [disabled status before restart](evidence/p3-restart/status-before-enable.txt) match plan hash `ee322702df2b0a3105f8f2b47c647c85c4c483c244f0450a394bf93036f0c3a7`. The existing watched acknowledgment for manual run `9bba8647-b9c3-4519-ad66-783da6c7d3fd` remains applicable; no new attestation was invented. Shipped `schedule enable` [succeeded](evidence/p3-restart/enable.txt), establishing epoch 3 at `2026-09-08T03:50:44.732006Z`. Its fixed 24-hour deadline is `2026-09-09T03:50:44.732006Z`, September 8 at 11:50 p.m. EDT.

The [launch record](evidence/p3-restart/launch.json) invokes `python -u tools/dev.py --instance p2-integration app schedule run --max-cycles 288` from the unchanged runtime checkout identified in that record, on `codex/feat-p2-p4`. It targets apps 10, 440 and 570, current players and name-only Store metadata every 300 seconds, one HTTP attempt, at most 1,728 requests and a maximum runtime of 86,400 seconds. Image `sha256:e5fcdd8a6748058c731b60e4a2354f81bb1c7698921c52a2d6cb44942af4feaf` is unchanged. New enrichment sources are outside this watched plan.

[The first-cycle snapshot](evidence/p3-restart/first-cycle.txt) records run `07395bba-bd54-4dc2-a7df-7beba5cf5e76`, three successful player requests and three successful Store requests, zero failed/uncertain attempts, and a running worker. Worker container `game-census-p2-integration-web-run-479929d74896` has restart policy `no`; its inherited website HTTP healthcheck reports unhealthy because the worker serves no HTTP. Durable job progress, process state and logs determine worker health; the real web and database are healthy. Initial host PID 23480 is informational and must be identity-checked before any process action.
