...
- [ ] Validate authenticated `steam_catalog_v2` against Steam after `sources.catalog_api_key` is configured locally. Start with `catalog sync --once --max-pages 1`: at most one request and 1,000 entries under the current policy. Record the real source outcome, retained capture and checkpoint; a bounded partial scan is not a completed catalog. This is the existing GC-B04 source-access gate and is not satisfied by synthetic lifecycle tests. FR-05, NFR-01, NFR-02.
