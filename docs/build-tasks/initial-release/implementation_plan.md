...
P3 live acceptance scope: use the existing isolated `p2-integration` instance and stored catalog page, pin apps 440/570, and adopt one catalog explorer under the existing three-member policy. The actual adopted explorer is app 10. Operator configuration uses a 300-second cadence, 10-second HTTP timeout, one attempt, name-only Store metadata, and an 86,400-second maximum worker duration. The admitted plan has six jobs per cycle and a 249-second conservative serialized cycle. After a person watches a successful manual cycle end to end, acknowledge that exact run and launch at most 288 cycles. Measure the fixed 24-hour window from the retained enable event, including all three games, missing/failed occurrences and stale app-time; target freshness is at least 95% at sample age below 600 seconds. Expected denominator is 259,200 app-seconds and 864 player occurrences. Record source outcomes, worker lifecycle, Docker resource samples and exact output; stop scheduling at completion or interruption. Do not change membership or collection settings during this fixed-cohort measurement. No additional code is planned for this gate; existing CLI, schedule coverage and Docker telemetry provide the evidence. P4 follows this acceptance result.

Authenticated validation repair: Steam returned HTTP 200, but the isolated database's `discovery_snapshot` still references frozen `capture_legacy_004`. NEW `migrations/009_catalog_reference.sql` validates a reference to current `capture_identity` before removing the obsolete foreign key on partitioned layouts; initialization applies it idempotently. MODIFY `tests/test_p2_integration.py` for schema 9. Existing frozen-copy regeneration creates tables without foreign keys and needs no change. MODIFY `tests/test_catalog.py` to reproduce the historical empty-upgrade path, exercise capture/checkpoint persistence and restore, and inject unexpected storage/report failures. MODIFY `tests/test_bootstrap.py` to clear the operator's catalog key from synthetic scratch settings; individual authenticated tests set their own fixture key. NEW `diagnostics.py` owns the existing allowlisted exception-context formatter extracted from `scheduler.py`; MODIFY `catalog.py` to persist the same secret-free context with the failed stage and report finalization errors on stderr. No exception messages, locals, credentials or payloads enter diagnostic output. The live repair requires a verified scratch restore before schema changes; source retries remain capped at one request and 1,000 entries with scheduling disabled. Run the catalog tests before and after, the full suite including scheduler diagnostics, and the ordinary catalog sync command after rebuilding. The checklist and walkthrough own status and exact evidence.

Current P3 cohort increment (FR-04/FR-05, NFR-01/NFR-02/NFR-03): NEW `cohort.py` provides deterministic stored-input selection, plan/status/reconcile CLI services and runtime effective membership. MODIFY `config.py` with an opt-in bounded `cohort` policy: configured `tracking.app_ids` stay pinned, active members use distinct promotion/demotion thresholds and fresh repeated evidence, and reserved exploration slots rotate through a bounded catalog cursor after minimum residence. Unknown/stale evidence never counts as zero. Configuration owns thresholds, dwell/reconcile times, candidate/evidence limits, change limits and discovery attempt reserves. Selection must pass the existing serialized scheduler/quota admission calculation, including extra discovery and watched replacement-run work, before adoption.

NEW additive `008_cohort.sql` owns immutable `cohort_event` adoption records and `tracking_stop` end events. Membership lives in the adoption ledger, not a rewritten configuration file. A reconcile preview performs only stored reads; explicit apply runs under the collection lock while scheduling is disabled, atomically records selected roles/cursor/policy, enrolls new members and closes removed members' intervals without deleting observations. Re-enrollment opens a new interval. Changed collection plans still require a successful watched manual run and matching acknowledgment before schedule enablement. Existing static cohorts remain the default.

Integration trace: MODIFY `cli.py` to register `cohort plan/status/reconcile --once --dry-run|--apply`, resolve the adopted cohort for initialization/collection/schedule commands and detect changed membership in a running worker; MODIFY `collector.py` and `scheduler.py` for the same effective membership and capacity reserves. MODIFY `db.py` and `cache.py` to expose/consume explicit tracking ends; MODIFY `contracts.py`, `queries.py`, `web.py`, `_state.html`, `game.html`, `compare.html`, `search.html` and `status.html` for retained-history versus active-membership states and safe cohort status. MODIFY `storage.py` only where frozen-copy regeneration needs the additive schema; verify that complete backup/restore retains the new canonical events. NEW `tests/test_cohort.py` covers fresh repeated evidence, hysteresis, zero, stale/absent counts, pinned membership, bounded/fair exploration, replacement limits, policy/capacity changes, dry-run, atomic apply/restart, manual/scheduled consumers, ended coverage/cache behavior, retained history and scratch restore. Extend `tests/test_p2_integration.py` schema-version checks to 8, and extend the `test_api.py`, `test_sources.py` and `test_config_cli.py` fixtures for the new read/bootstrap interface without weakening their failure checks. Extend `tools/browser_read_check.py --cohort-only` for adopted roles, stopped history and changed-policy desktop/mobile inspection. `latest_app_name` in migration 008 supplies one derived name rule so enrollment cannot hide catalog names. Turning the policy off requires an explicit disabled-policy reconciliation before runtime collection resumes. Update README/PRS/brief/checklist/walkthrough with shipped interfaces and exact evidence.

Execution order: (1) typed policy and deterministic selection, (2) stored inputs/adoption ledger and interval/cache integration, (3) all runtime consumers and admission accounting, (4) integration/restore/browser and live bounded acceptance. The cohort task remains in progress until all four applicable implementation/verification stages are complete. Authenticated catalog and watched next-cohort evidence remain separate gates; synthetic inputs do not satisfy them.
