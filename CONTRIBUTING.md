...
Configuration defaults and bounds belong to `src/game_census/config.py`. Runtime pins belong to `build/toolchain.lock.json`; Python dependencies belong to `uv.lock`. Deliberate dependency changes need their own verification. Check derived files with `python tools/sync_toolchain.py --check` and `python tools/sync_ci.py --check`. Compare canonical wheel artifacts and check installed assets with `python tools/repro_build.py`. The generator normalizes disposable source copies to LF and fixes wheel ZIP metadata without changing member contents. Use `python tools/dev.py --instance contribution test-artifact --wheel PATH` to verify historical database upgrades and recovery using the installed wheel.
